...
export class InmobiliariaService {
  ...
  async create(
    createInmobiliariaDto: CreateInmobiliariaDto, registerCuentaDto: RegisterCuentaDto): Promise<Inmobiliaria> {
    return await this.inmobiliariaRepository.manager.transaction(
      async (transactionalEntityManager) => {
        // Crear la cuenta dentro de la transacción
        const cuenta = await this.authService.register(
          registerCuentaDto,
        ...
        const inmobiliariaExistente = await transactionalEntityManager.findOne(
          Inmobiliaria,
          {
            where: { direccion },
          },
        ...
        if (!localidad) {
          throw new NotFoundException(
            `Localidad con id ${localityId} no existe`,
          );
        }

        // Verificar que la cuenta existe (por consistencia)
        const cuentaExistente = await transactionalEntityManager.findOne(
          Cuenta,
          {
            where: { id: cuentaId },
          },
  ...
  async createAfterV2(createInmobiliariaDto: CreateInmobiliariaDto, registerCuentaDto: RegisterCuentaDto, clienteIp: string,): Promise<Inmobiliaria> {
    return await this.inmobiliariaRepository.manager.transaction(
      async (transactionalEntityManager: EntityManager) => {
        // 1. VERIFICAR V2 Y CREAR CUENTA
        // Llama al nuevo método que verifica el token V2 y luego realiza el registro
        ...
        const {
          ...
          numeroTelefono,
          localidad: localityId,
        } = createInmobiliariaDto; // Verificar si la dirección ya existe
  ...
  async findByCuenta(cuentaId: number): Promise<Inmobiliaria> {
    ...
    if (!inmobiliaria) {
      throw new NotFoundException(
        `No se encontró una inmobiliaria asociada a la cuenta con ID ${cuentaId}`,
      );
    }
    return inmobiliaria;
  }
  ...
  async update(
    ...
    if (
      updateInmobiliariaDto.direccion &&
      updateInmobiliariaDto.direccion !== inmobiliariaToUpdate.direccion
    ) {
      const inmobiliariaConMismaDireccion =
        await this.inmobiliariaRepository.findOneBy({
          direccion: updateInmobiliariaDto.direccion,
        });
      ...
      if (
        inmobiliariaConMismaDireccion &&
        inmobiliariaConMismaDireccion.id !== id
      ) {
        throw new ConflictException(
          'La nueva dirección ya está registrada en otra inmobiliaria',
        );
      }
    }
    ...
    if (updateInmobiliariaDto.localidad !== undefined) {
      const localidad = await this.localidadRepository.findOne({
        where: { id: updateInmobiliariaDto.localidad },
      });
      if (!localidad) {
        throw new NotFoundException(
          `Localidad con id ${updateInmobiliariaDto.localidad} no existe`,
        );
      }
  ...
  async updatePlan(
    ...
    if (!inmobiliaria) {
      throw new NotFoundException(
        `Inmobiliaria con ID ${idInmobiliaria} no encontrada`,
      );
    }
