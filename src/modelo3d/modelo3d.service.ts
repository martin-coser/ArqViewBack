...
export class Modelo3DService {
  ...
  async upload(
    ...
    if (!propiedad) {
      ...
      throw new NotFoundException(
        `Propiedad con ID ${uploadModelo3DDto?.propiedad} no encontrada.`,
      );
    }
    ...
    try {
      await this.modelo3DRepository.save(modelo3D);
      // Eliminar archivo temporal solo si todo salió bien
      if (fs.existsSync(file.path)) {
        fs.unlinkSync(file.path);
      }
    } catch (error) {
      // Si falla el guardado, eliminar archivo temporal
      if (fs.existsSync(file.path)) {
        fs.unlinkSync(file.path);
      }
      throw error;
    ...
    return { imageUrl: modelo3D.filePath };
  }
  ...
  async findByPropiedad(propiedadId: number): Promise<Modelo3D[]> {
    if (await this.inmobiliariaService.esPremium(propiedadId)) {
      ...
      return await this.modelo3DRepository.find({
